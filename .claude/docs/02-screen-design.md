...
- 来院日マーク表示（日付下にドット）
  - 次回来院日: bg-[#1E40AF]（青ドット）
  - 前回来院日: bg-[#10B981]（緑ドット）
- 月ナビゲーション（前月/翌月/今日ボタン）
- 日付タップで詳細パネル表示（完了/未実施の運動一覧、セット数・回数・実施時刻）
- 来院日選択時は詳細パネル上部に来院日バナー表示
- 凡例表示（全完了/一部実施/未実施/次回来院/前回来院）
- 実装:
...
- チェックボックス形式
- 効率的な記録入力
- 本日の実施記録を取得し、実施済み/未実施を区別表示
  - 未実施: チェックボックス付き（選択可能）、上部に表示
  - 実施済み: 緑系UI（bg-green-50, CheckCircle2アイコン）、下部に表示、選択不可
- ヘッダーに残り件数バッジ（残りN件 / すべて完了）
- 全運動実施済み時はボタンが「すべて実施済みです」で disabled
...
### S-02: ダッシュボード
- 担当患者一覧
  - 患者行クリックで患者詳細（S-04）へ遷移
  - 「測定値入力」ボタンで測定値入力（S-05）へ遷移
  - 「メニュー設定」ボタンで運動メニュー設定（S-06）へ遷移
- 本日の来院予定
- KPI表示（担当患者数、運動実施数など）
...
- **新規患者登録機能**（マネージャーのみ）✅
  ...
  - 登録ダイアログ/モーダル
  - 必須項目: 氏名、メール、生年月日、初期パスワード（患者コードはサーバーで自動採番: USR + 連番3桁以上）
...
- 患者ごとの運動メニュー割当
- 疾患・状態に応じた推奨表示
- 2段階アコーディオン: 大分類（体幹・脊柱/上肢/下肢）→ 中分類（膝・下腿/腰椎等）→ 運動一覧
  - 大分類: デフォルトで全展開
  - 中分類: 割当済み運動がある場合のみ自動展開、ワンタップで開閉
