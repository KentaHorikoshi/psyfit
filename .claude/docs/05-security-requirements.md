...
Rack::Attack によるAPIレート制限。セッション情報に基づき利用者・職員で異なる制限を適用。
...
**制限超過時**: 429 Too Many Requests + `Retry-After` ヘッダー + 監査ログ記録

**不正リクエスト遮断**: SQLi/XSSパターン検出 → Fail2Ban（3回/10分で1時間遮断） → 403 Forbidden

**レスポンスヘッダー**: 全APIレスポンスに `X-RateLimit-Limit` / `X-RateLimit-Remaining` / `X-RateLimit-Reset` を付与
