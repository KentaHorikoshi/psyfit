...
- [ ] 当日体調未記録の場合、赤いバッジ（パルスアニメーション付き）が表示される
- [ ] 当日体調記録済みの場合、バッジが表示されない
- [ ] バッジ表示時にaria-labelが「体調を入力（未入力）」になる
...
- [ ] 当日初回運動 かつ 体調未記録 → 3秒後に体調入力画面（U-14）に自動遷移
- [ ] 当日初回運動 かつ 体調未記録 → 「続ける」ボタンで即時遷移（U-14へ）
- [ ] 当日2回目以降 または 体調記録済み → 3秒後にホーム画面（U-02）に自動遷移
- [ ] 当日2回目以降 または 体調記録済み → 「続ける」ボタンで即時遷移（U-02へ）
- [ ] API失敗時はホーム画面（U-02）にフォールバック遷移
...
#### ログイン〜運動実施フロー（2回目以降 or 体調記録済み）

```
ホーム(U-02) → 運動メニュー選択(U-03) → 運動実施(U-04) →
祝福(U-13) → ホーム(U-02)
```

- [ ] 当日2回目以降の運動完了時、祝福画面からホーム画面に直接遷移する
- [ ] 当日体調記録済みの場合、祝福画面からホーム画面に直接遷移する
